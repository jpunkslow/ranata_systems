...
class Sales_Quotation_model extends Crud_model {
    ...
    function get_details($options = array()){
        ...
        $data = $this->db->query("SELECT * FROM sales_quotation WHERE  deleted = 0  ".$where." ORDER BY id DESC");
        return $data;
    }

    function get_quotation_total_summary($invoice_id = 0) {
        $invoice_items_table = $this->db->dbprefix('sales_quotation_items');
        $invoices_table = $this->db->dbprefix('sales_quotation');
    ...
    function get_quotation_value($invoice_id = 0){
