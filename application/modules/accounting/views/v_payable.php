...
url="<?php echo site_url('accounting/payable/ajax_list'); ?>" 
...
<thead>
	<tr>
		...
		<th data-options="field:'akun_id_txt',width:'20', halign:'center', align:'left'" >Untuk Akun</th>
...
function create(){
	...
	url = '<?php echo site_url('accounting/payable/create'); ?>';
}
...
function update(){
	...
	if(row){
		...
		url = '<?php echo site_url('accounting/payable/update'); ?>/' + row.id;
		$('#jumlah ~ span input').keyup(function(){
			var val_jumlah = $(this).val();
			$('#jumlah').numberbox('setValue', number_format(val_jumlah));
		});
	...
	if (row){ 
		$.messager.confirm('Konfirmasi','Apakah Anda akan menghapus data kode transaksi : <code>' + row.id_txt + '</code> ?',function(r){  
			if (r){  
				$.ajax({
					type	: "POST",
					url		: "<?php echo site_url('accounting/payable/delete'); ?>",
					data	: 'id='+row.id,
					success	: function(result){
						var result = eval('('+result+')');
						$.messager.show({
...
function cetak () {
	...
	var win = window.open('<?php echo site_url("accounting/payable/cetak_laporan/?kode_transaksi=' + kode_transaksi + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai + '"); ?>');
	if (win) {
		win.focus();
	} else {
