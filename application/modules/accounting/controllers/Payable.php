...
class Payable extends MY_Controller {
    ...
    public function __construct() {
		parent::__construct();	
		$this->load->model('accounting/Payable_model');
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->Payable_model->get_data_kas();
		$this->data['akun_id'] = $this->Payable_model->get_data_akun();

		$this->template->rander('v_payable', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->Payable_model->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->Receivable_model->get_jenis_kas($r->dari_kas_id);	
	...
	public function update($id=null) {
		...
		if($this->Payable_model->update($id)) {
	...
	public function delete() {
		...
		if($this->Payable_model->delete($id))
		{
	...
	function cetak_laporan() {
		$pengeluaran = $this->Payable_model->lap_data_pengeluaran();
